    ...
    switch (reason) {
        ...
        case LWS_CALLBACK_RAW_CONNECTED: {
            ...
            if (lws_get_opaque_parent_data(wsi) == NULL) {
                lwsl_warn("[raw] connected, tunnel is closed");
                return -1;
            }
            if (wss_tunnel == NULL) {
                lwsl_warn("[raw] connected, tunnel is null");
                return -1;
            }
        ...
        case LWS_CALLBACK_RAW_RX: {
            struct wss_tunnel *wss_tunnel = user;
            if (lws_get_opaque_parent_data(wsi) == NULL) {
                lwsl_warn("[raw] received, tunnel is closed");
                return -1;
            }
            if (wss_tunnel == NULL) {
            ...
            if (wss_tunnel->wss_state == STATE_CLOSED) {
                lwsl_warn("[raw] received %u from raw for peer %d, tunnel is closed",
                            (uint16_t) len, wss_tunnel->peer_port);
                return -1;
            }
            if (len + wss_tunnel->raw_len > RX_BUFFER_SIZE) {
                lwsl_err("[raw] received %u from raw for peer %d, pending: %u, buffer: %d",
...
static int callback_wss_server(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len) {
    switch (reason) {
        case LWS_CALLBACK_ESTABLISHED: {
            ...
            if (wss_tunnel == NULL) {
                lwsl_err("[wss] established, tunnel is null");
                return -1;
            }
            if (wss_tunnel->wss_state) {
                lwsl_err("[raw] established, tunnel is initialized");
                return -1;
            }
        ...
        case LWS_CALLBACK_RECEIVE: {
            struct wss_tunnel *wss_tunnel = user;
            if (lws_get_opaque_parent_data(wsi) == NULL) {
                lwsl_err("[wss] received, tunnel is closed");
                return -1;
            }
            if (wss_tunnel == NULL) {
                lwsl_err("[wss] received, tunnel is null");
                return -1;
            }
            if (len + wss_tunnel->wss_len > RX_BUFFER_SIZE) {
                lwsl_err("[wss] received %u from peer %d, pending: %u, buffer: %d",
        ...
        case LWS_CALLBACK_SERVER_WRITEABLE: {
            struct wss_tunnel *wss_tunnel = user;
            if (lws_get_opaque_parent_data(wsi) == NULL) {
                lwsl_err("[wss] writable, tunnel is closed");
                return -1;
            }
            if (wss_tunnel == NULL) {
                lwsl_err("[wss] writable, tunnel is null");
                return -1;
            }
        ...
        case LWS_CALLBACK_CLOSED: {
            ...
            struct wss_tunnel *wss_tunnel = user;
            if (wss_tunnel == NULL) {
                lwsl_warn("[wss] closed, tunnel is null");
                break;
            }
            ...
            lws_set_opaque_parent_data(wsi, NULL);
            break;
        }
        default:
            break;
